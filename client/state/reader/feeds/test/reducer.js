...
describe( 'reducer', () => {
	describe( 'items', () => {
		...
		test( 'should serialize feed entries', () => {
			...
			expect( items( unvalidatedObject, { type: SERIALIZE } ) ).toEqual( unvalidatedObject );
		} );
		...
		test( 'should not serialize errors', () => {
			...
			expect( items( stateWithErrors, { type: SERIALIZE } ) ).toEqual( {
				12: { feed_ID: 12 },
			} );
		} );
		...
		test(
			...
			sinon.test( function() {
				...
				expect( items( unvalidatedObject, { type: DESERIALIZE } ) ).toEqual( {} );
			} )
		);
	...
	describe( 'lastFetched', () => {
		test( 'should update the last fetched time on request success', () => {
			...
			expect( typeof lastFetched( original, action )[ '1' ] ).toBe( 'number' );
		} );
